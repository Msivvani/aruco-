...
from apriltags import Detector as ApriltagDetector


# utils import python code
from utils import aruco_display

def pose_estimation_at(rgb_frame: np.array, depth_frame: np.array, at_detector: ApriltagDetector, marker_size: float,
    ...
    at_detector - Duckietown AR tag detector object
    marker_size - Size of the AR tags in meters
    ...
    return:
    frame_processed - The frame with the axis drawn on it
    ...
    # Detect AR tags using Duckietown detector
    ...
    if tags:
        logger("Detected {} markers.".format(len(tags)))

        for tag in tags:
            # Estimate pose using Duckietown detector
            tag_id = tag.tag_id
            corners = tag.corners
            center = tag.center
            pose_r = tag.pose_r  # Rotation matrix of the tag
            pose_t = tag.pose_t  # Translation vector of the tag

            # Example of drawing the detected tag on the image (adjust as needed)
            cv2.polylines(frame_processed, [np.array(corners, dtype=np.int32)], True, (0, 255, 0), 2)

            # Example of computing pose (adjust as needed)
            if depth_frame is not None:
                # Assuming depth frame to pointcloud centroid computation
                centroid = depth_to_pointcloud_centroid(depth_frame=depth_frame, intrinsic_matrix=matrix_coefficients, corners=corners)
                pose = Pose()
                pose.position.x = float(centroid[0])
                pose.position.y = float(centroid[1])
                pose.position.z = float(centroid[2])
            else:
                # Assuming direct translation vector from Duckietown detector
                pose = Pose()
                pose.position.x = float(pose_t[0])
                pose.position.y = float(pose_t[1])
                pose.position.z = float(pose_t[2])

            # Assuming rotation matrix from Duckietown detector
            rvec, _ = cv2.Rodrigues(pose_r)
            quat = quaternion_from_matrix(rvec)
            ...
            # Add pose and marker ID to messages
            pose_array.poses.append(pose)
            markers.poses.append(pose)
            markers.marker_ids.append(tag_id)

            # Draw frame axes
            frame_processed = cv2.drawFrameAxes(frame_processed, cameraMatrix=matrix_coefficients,
                                                distCoeffs=distortion_coefficients, rvec=rvec, tvec=pose_t,
